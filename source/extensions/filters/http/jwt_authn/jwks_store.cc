...
class JwksDataImpl : public JwksStore::JwksData, public Logger::Loggable<Logger::Id::filter> {
public:
...
class JwksStoreImpl : public JwksStore {
public:
...
JwksStorePtr JwksStore::create(
    const ::envoy::config::filter::http::jwt_authn::v2alpha::JwtAuthentication& config) {
  return JwksStorePtr(new JwksStoreImpl(config));
}

...
class JwksStore;
typedef std::unique_ptr<JwksStore> JwksStorePtr;

/**
 * Interface to access all configured Jwt rules and their cached Jwks objects.
 * Its usage:
 *     auto jwks_store = JwksStore::create(Config);
 *
 *     // for a given jwt
 *     auto jwks_data = jwks_store->findByIssuer(jwt->getIssuer());
...
public:
  ...
  public:
    ...
    // Get the stored config: JWT rule.
